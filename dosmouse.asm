...
proc 	Multiplexeur_ED	Far		;nvelle int. (Multiplexeur)
		sti			;autre int. autorisées
		pushf
		cmp ah,0EDh		;test si on s' adresse à DOSMOUSE
		je mux_ok		;si oui ...
		popf
		jmp cs:Old_Multiplexeur	;sinon appel l' ancien Multiplexeur
mux_ok:
		popf
		or al,al		;al= n° de fonction
		jnz saut_mux_1		;si al <> fonction test
		mov al,0EDh		;indiquer que D.Mouse est installé
fin_mux:
		iret			;Retour
saut_mux_1:
...
proc	mouse_ed Far		;détermine si la souris est affichée ou non
		pushf
		or ax,ax
		jnz st_mouse0
...
st_mouse0:
		...
		jne st_mouse1
		inc cs:etat_mouse	;(on incrément le drapeau)
		jmp st_mouse_fin
st_mouse1:
		cmp ax,02h		;fonction cache souris
		jne st_mouse2
		dec cs:etat_mouse	;(décrémente le drapeau)
		jmp st_mouse_fin
st_mouse2:
...
proc	int_clavier far		;simule l' appuie de touches sur le clavier
		sti				; pour envoyer la sélection
		cmp cs:can_write,00		; vérifie si on peut écrire
		jne write_selection
		cli
...
write_selection:
		...
		lea bx,selection	;sinon remet le pointer à l'origine
		mov cs:ptr_sel,bx
		pop bx
		mov cs:can_write,00	;plus besoin d'ecrire (tfr terminé)
...
do_tfr_car:
		mov cs:can_run,0	;désactive le controleur DosMouse
		...
		jmp cs:old_int_clavier		;sinon appel ancien drv clavier
	fct0:
		mov al,cs:[bx]		;tranfert le car pointé par bx
		xor ah,ah		;pas de scancode (souvent inutile)
		inc cs:ptr_sel		;passe au car suivant (appel suivant)
		pop bx
		iret			;retour
	fct1:
		mov al,cs:[bx]		;transfert le car pointé par bx
		xor ah,ah		;pas de scancode
		pop bx
		cmp ah,1		;ZF=0 => car présent
		ret 2			;retour en conservant les flags
endp

proc	tache_de_fond far	;procedure appelée quand le DOS ne fait rien
		sti				;int autorisées
		pushf
		PUSHT
		
		cmp cs:etat_mouse,0		;la souris est-elle éteinte?
		jz I_can			;oui
Ho_no:
		jmp no_I_can_not		;non, on sort
I_can:
		cmp cs:just_write,0
		jne Ho_no			;vient-on juste d' écrire?

		call does_mouse_move
		or  cx,cx
		jz  ho_no

		mov ah,0Fh			;lit mode vidéo actuelle
		int 10h
		cmp al,03			;seul le mode texte est suporté
		jna I_can1
...
I_can1:
		...
		xchg al,ah			;bh=page_vidéo
		mul bh				;calcul offset de l'écran car:
		mov dh,50d			;off=(col*25*2)*page_page_vidéo
		mul dh
		...
		mov ah,01h
		mov cx,2020h			;éteint le curseur texte
		int 10h
		test cs:args_flags,1000b	;la souris part d'où ?
		...
		push ax				;sauv résultat sur la pile
		xor ah,ah
...
;********************************** la partie qui suit ne reste pas en mémoire
Debut:
		mov ah,30h
		int 21h
		cmp al,3
...
Doit_installer:
		...
		jmp erreur
drv_souris_ok:
		;************** Ici les échanges d'interruptions
		mov ax,0352Fh
		int 21h
...
dm_inst		db 'DosMouse installé',10,13,'$'
no_mouse	db 'Installation stoppée: le driver souris n'' est pas en'
		db ' mémoire',10,13,'$'
bascule		db 'DosMouse reconfiguré',10,13,'$'
dj_inst		db 'DosMouse déjà installé',10,13,'$'
Usage		db 10,13,'Syntaxe: DOSMOUSE [/options+|-] [/v] [/r]',10,13
		db 10,13,' V = affiche le mode DosMouse (visualisation des'
		db ' réglages)'
		db 10,13,'option peut être l''une des quatre lettres: P,E,L,C'
		db 10,13,' P = ajout automatique du Point d''extension sur'
		db ' les fichiers'
		db 10,13,' E = simulation de l''appuie de la touche Entrée'
		db ' après une séléction'
		db 10,13,' L = déplacement de la sélection avec la souris en'
		db ' mode Ligne horizontale'
		db 10,13,' C = repositionnement de la souris à l''emplacement'
		db ' actuel du Curseur'
		db 10,13,' (chaque option doit être suivie de + ou - sauf pour'
		db ' ''v'' et ''r'')'
		db 10,13,' + = activation de l''option,    - = désactivation.'
		db 10,13
		db 10,13,' exemple: DOSMOUSE /p+ /e- /v /c+'
		db 10,13,'       (par défaut, DosMouse désactive chaque'
		db ' option)',10,13,'$'
...
dm_Etat		db 10,13,'Etat de dosmouse:'
		...
		db ' les fichiers              : '
etat_pnt	db 'Non'
		db 10,13,' simulation de l''appuie de la touche Entrée après'
		db ' une sélection       : '
etat_Ent	db 'Non'
		db 10,13,' déplacement de la sélection avec la souris en mode'
		db ' Ligne horizontale : '
etat_sel	db 'Non'
		db 10,13,' repositionnement de la souris à l''emplacement'
		db ' actuel du Curseur      : '
etat_org	db 'Non'
		db 10,13,'$'
